...
from utils.hashing import hash_row
...
def build_sat_notificacao_evento(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return sat[[
        'hk_notificacao', 'hashdiff', 'tp_not', 'dt_notific', 'sem_not', 'nu_ano',
        'dt_sin_pri', 'sem_pri', 'dt_invest', 'load_date', 'record_source'
...
def build_sat_notificacao_pessoa(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sat['hashdiff'] = sat.apply(lambda row: hash_row(row, attrs), axis=1)
    return sat[[
        'hk_notificacao', 'hashdiff', 'nu_idade_n', 'cs_sexo', 'cs_gestant',
        'cs_raca', 'cs_escol_n', 'load_date', 'record_source'
    ]].drop_duplicates().reset_index(drop=True)
...
def build_sat_notificacao_encerramento(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sat['hashdiff'] = sat.apply(lambda row: hash_row(row, attrs), axis=1)
    return sat[[
        'hk_notificacao', 'hashdiff', 'classi_fin', 'criterio', 'tpautocto',
